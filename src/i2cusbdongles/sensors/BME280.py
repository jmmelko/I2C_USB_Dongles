...
#ISS         RX 10:47:54   [  1] [  1]                       == b'\xd6'              == D6              # success
    ...
    def __init__(self, BME280):
        self.dongle  = BME280["dngl"]    # A Dongle object among "ELVdongle", "IOW-DG", "ISSdongle"
    ...
    def BME280Init(self):
        ...
        data    = [0xE0, 0xB6]
        rbytes  = 0
        answ    = self.dongle.askDongle(self.addr, data, rbytes, name=self.name, info="Soft Reset")
        ...
        data    = [0xf2, 0x05]
        rbytes  = 1
        answ    = self.dongle.askDongle(self.addr, data, rbytes, name=self.name, info="ctrl_hum")

        # Calibration Data calib00...calib25 (0x88 ... 0x9F) 24 values
        data    = [0x88]
        rbytes  = 24
        self.cal1    = self.dongle.askDongle(self.addr, data, rbytes, name=self.name, info="get cal1")

        # Calibration Data calib26...calib41 (0xA1 ) 1 value
        data    = [0xA1]
        rbytes  = 1
        self.cal2    = self.dongle.askDongle(self.addr, data, rbytes, name=self.name, info="get cal2")

        # Calibration Data calib26...calib41 (0xe1 ... 0xe7) 7 values
        data    = [0xe1]
        rbytes  = 7
        self.cal3    = self.dongle.askDongle(self.addr, data, rbytes, name=self.name, info="get cal3")
    ...
    def BME280getTPH(self):
        ...
        data    = [0xf4, 0xd6]
        rbytes  = 1
        answ    = self.dongle.askDongle(self.addr, data, rbytes, name=self.name, info="ctrl_meas")
        ...
        data    = [0xf7]
        rbytes  = 8
        answ    = self.dongle.askDongle(self.addr, data, rbytes, name=self.name, info="Get data F7...FE", end="")

        press_raw, temp_raw, hum_raw = self.__BME280getRawData(answ)

        press_semi = press_raw / 1000 * 3  # this gives roughly correct values
        temp_semi  = temp_raw  / 10000 / 2 # although not useful enough due
        hum_semi   = hum_raw   / 1000      # mainly to interaction between signals

        t, p, h = readBME280All(self.cal1, self.cal2, self.cal3, press_raw, temp_raw, hum_raw)
        util.ncprint("  Result: T: {:6.2f}, P: {:6.2f}, H: {:6.2f}".format(t, p, h), color=glob.TDEFAULT)

        return t, p, h, temp_semi, press_semi, hum_semi


    def __BME280getRawData(self, rec):
        """ cals raw  press, temp, hum"""
    ...
    def BME280runAllFunctions(self):
        ...
        data    = [0xD0]
        rbytes  = 1
        answ    = self.dongle.askDongle(self.addr, data, rbytes, num = True, name=self.name, info="get ID")
        ...
        data    = [0xE0, 0xB6]
        rbytes  = 1
        answ    = self.dongle.askDongle(self.addr, data, rbytes, num = True, name=self.name, info="Soft Reset")
        ...
        data    = [0xf2, 0x05]
        rbytes  = 1
        answ    = self.dongle.askDongle(self.addr, data, rbytes, num = True, name=self.name, info="ctrl_hum")
        ...
        data    = [0xf3]
        rbytes  = 1
        answ    = self.dongle.askDongle(self.addr, data, rbytes, num = True, name=self.name, info="status")
        ...
        data    = [0xf4]
        rbytes  = 1
        answ    = self.dongle.askDongle(self.addr, data, rbytes, num = True, name=self.name, info="ctrl_meas")
        ...
        data    = [0xf5]
        rbytes  = 1
        answ    = self.dongle.askDongle(self.addr, data, rbytes, num = True, name=self.name, info="get config")

        # Read calibration data
        # Calibration Data calib00...calib25 (0x88 ... 0x9F) 24 values
        data    = [0x88]
        rbytes  = 24
        self.cal1    = self.dongle.askDongle(self.addr, data, rbytes, num = True, name=self.name, info="get cal1")

        # Calibration Data calib26...calib41 (0xA1 ) 1 value
        data    = [0xA1]
        rbytes  = 1
        self.cal2    = self.dongle.askDongle(self.addr, data, rbytes, num = True, name=self.name, info="get cal2")

        # Calibration Data calib26...calib41 (0xe1 ... 0xe7) 7 values
        data    = [0xe1]
        rbytes  = 7
        self.cal3    = self.dongle.askDongle(self.addr, data, rbytes, num = True, name=self.name, info="get cal3")
        ...
        data    = [0xf7]
        rbytes  = 8
        answ    = self.dongle.askDongle(self.addr, data, rbytes, num = True, name=self.name, info="data F7...FE")

        t, p, h = readTSL2591All(cal1, cal2, cal3, pres_raw, temp_raw, hum_raw)
        util.fncprint(u"FULL: p, t, h: {:5.2f},  {:8.2f},  {:5.2f}".format( t, p, h))
        util.fncprint(u"SEMI: p, t, h: {:5.2f},  {:8.2f},  {:5.2f}".format( temp_semi, pres_semi, hum_semi))
...
def readBME280All(cal1, cal2, cal3, pres_raw, temp_raw, hum_raw):
